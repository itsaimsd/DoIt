...
// const TaskControls = ({ isCompleted, toggleComplete, isImportant, task, toggleFavorite }) => {
//   if (!task) return null;
...
//         <span className="task-text">{task.text}</span>
//         <FontAwesomeIcon
//           icon={isImportant ? solidStar : regularStar}
//           className="star-icon"
  ...
  return (
    <div className="task-controls">
      ...
      <div className="task-name">
        ...
        />
        <span className="task-text">{task.text}</span> {/* Render the task name */}
        {/* Important Star */}
        <FontAwesomeIcon
          icon={task.important ? solidStar : regularStar} // Toggle between filled and unfilled star
          className={`star-icon ${task.important ? "important" : ""}`} // Add class for styling
          onClick={() => {
            toggleFavorite(task.id); // Toggle importance when the star is clicked
          }}
        />
